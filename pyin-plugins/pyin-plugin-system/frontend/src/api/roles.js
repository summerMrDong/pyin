...
export async function fetchRoles(params = {}) {
  const result = await httpRequest(`/api/roles${buildQuery(params)}`)
  return unwrapResult(result, '加载角色列表失败')
}

export async function fetchRoleOptions() {
  const result = await httpRequest('/api/roles/options')
  return unwrapResult(result, '加载角色选项失败')
}

export async function fetchRoleDetail(id) {
  const result = await httpRequest(`/api/roles/${id}`)
  return unwrapResult(result, '加载角色详情失败')
}

export async function createRole(payload) {
  const result = await httpRequest('/api/roles', {
    method: 'POST',
    body: payload
  })
...
export async function fetchRolePermissions(id) {
  const result = await httpRequest(`/api/roles/${id}/permissions`)
...
export async function fetchRoleResources(id) {
  const result = await httpRequest(`/api/roles/${id}/resources`)
...
export async function fetchRoleUsers(id) {
  const result = await httpRequest(`/api/roles/${id}/users`)
